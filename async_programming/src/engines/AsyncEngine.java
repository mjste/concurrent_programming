...
public class AsyncEngine implements IEngine{
    ...
    public void run() throws InterruptedException {
        ...
        for (int i = 0; i < argParser.producers; i++) {
            AsyncProducer asyncProducer = new AsyncProducer(scheduler, bufferMonitor, i, argParser.bound, false, argParser.workToDo);
            asyncProducerList.add(asyncProducer);
        }
        for (int i = 0; i < argParser.consumers; i++) {
            AsyncConsumer asyncConsumer = new AsyncConsumer(scheduler, bufferMonitor, i, argParser.bound, false, argParser.workToDo);
